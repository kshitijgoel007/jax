...
template <typename OpTy>
LogicalResult yield_op_rule_impl(RewriteContext &ctx, OpTy op,
                             const ArrayRef<Layout> layouts_in,
                             const ArrayRef<Layout> layouts_out) {
...
LogicalResult scf_yield_rule(RewriteContext &ctx, Operation &op,
  ...
  return yield_op_rule_impl(ctx, yield_op, layouts_in, layouts_out);
}

LogicalResult tpu_yield_rule(RewriteContext &ctx, Operation &op,
                             const ArrayRef<Layout> layouts_in,
                             const ArrayRef<Layout> layouts_out) {
  auto yield_op = cast<tpu::YieldOp>(op);
  return yield_op_rule_impl(ctx, yield_op, layouts_in, layouts_out);
}
...
const llvm::StringMap<rule_type> &rules() {
  static auto rules = new llvm::StringMap<rule_type>{
      ...
      {tpu::YieldOp::getOperationName(), tpu_yield_rule},
